...
export class AuthHttp {
  ...
  constructor(options: AuthConfig, private http: Http, private router: Router, private defOpts?: RequestOptions) {
...
export const AUTH_PROVIDERS: any = [
  provide(AuthHttp, {
    ...
    useFactory: (http: Http, router: Router, options: RequestOptions) => {
      return new AuthHttp(new AuthConfig(), http, router, options);
    }
  })
];
...
export function provideAuth(config = {}): any[] {
  return [
    provide(AuthHttp, {
      deps: [Http, RequestOptions],
      useFactory: (http: Http, router: Router, options: RequestOptions) => {
        return new AuthHttp(new AuthConfig(config), http, router, options);
      }
    })
  ];
}
